...
HWND CConEmuChild::Create()
{
	...
	//if (!RegisterClass(&wc)) {
	//	ghWndDC = (HWND)-1; // чтобы родитель не ругалс€
...
LRESULT CALLBACK CConEmuChild::ChildWndProc(HWND hWnd, UINT messg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SETFOCUS:
		SetFocus(ghWnd); // ‘окус должен быть в главном окне!
		return 0;
    ...
    default:
		// —ообщение приходит из ConEmuPlugin
		if (messg == mn_MsgTabChanged) {
			if (gSet.isTabs) {
				...
				// иначе во врем€ запуска PID фара еще может быть не известен...
				//gConEmu.OnTimer(0,0); не получилось. индекс конмана не мен€лс€, из-за этого индекс активного фара так и осталс€ 0
				TabBar.Retrieve();
			}
		}
...
WARNING("!!! Ќа врем€ скроллировани€ необходимо установить AutoScroll в TRUE, а при отпускании ползунка - вернуть старое значение!");
TODO("» вообще, скроллинг нужно передавать через pipe");
...
HWND CConEmuBack::Create()
{
	...
	if (!RegisterClass(&wc)) {
		...
		mh_WndBack = (HWND)-1; // чтобы родитель не ругалс€
		mh_WndScroll = (HWND)-1;
