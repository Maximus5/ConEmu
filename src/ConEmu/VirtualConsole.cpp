...
{
	Free(true);
	pVCon = NULL; //??? так на вс€кий случай
}
...
{
	// ј вообще, имеет смысл отрисовывать?
	if (vc->bgBmp.cx>inX && vc->bgBmp.cy>inY)
		BitBlt(vc->hDC, inX, inY, inWidth, inHeight, vc->hBgDc, inX, inY, SRCCOPY);
	if (vc->bgBmp.cx<(inX+inWidth) || vc->bgBmp.cy<(inY+inHeight))
	{
		if (hBrush0==NULL) {
		...
		RECT rect = {max(inX,vc->bgBmp.cx), inY, inX+inWidth, inY+inHeight};
		if (!IsRectEmpty(&rect))
			FillRect(vc->hDC, &rect, hBrush0);

		if (vc->bgBmp.cx>inX) {
			rect.left = inX; rect.top = max(inY,vc->bgBmp.cy); rect.right = vc->bgBmp.cx;
			if (!IsRectEmpty(&rect))
				FillRect(vc->hDC, &rect, hBrush0);
		}
...
void VirtualConsole::SelectBrush(HBRUSH hNew)
{
	if (!hNew) {
		if (hOldBrush)
...
// ¬озвращает true, если процедура изменила отображаемый символ
...
{
    #ifdef _DEBUG
	DcDebug dbg(&hDC, ahDc); // дл€ отладки - рисуем сразу на канвасе окна
	#endif
		...
		if (updateText || updateCursor)
		{
			...
			for (; pos >= 0; ConCharLine -= TextWidth, ConAttrLine -= TextWidth, pos -= gSet.LogFont.lfHeight, --row)
			{
				...
				for (int j2=0; j < end; j = j2)
				{
					...
					// ¬озможно это тоже нужно вынести в GetCharAttr?
					...
					if (gSet.isForceMonospace)
					{
						...
						if (c == 0x20 || !c) {
							//c = 0x3000;//0x2003; -- ј это что за изврат?
						} else if (c <= 0x7E && c >= 0x21) {
							// ћл€, и нахрена поднимать код.таблицу дл€ нижней половины ASCII символов?
							//c += 0xFF01 - 0x21;
						}
						*/
