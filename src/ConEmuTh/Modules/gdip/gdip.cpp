...
struct GDIPlusImage
{
	...
	//	// IStream используетс€ в процессе декодировани€, поэтому его нужно держать созданным	
	//	strm = new MStream();
	//	if (strm) {
	...
	bool Open(bool bVirtual, const wchar_t *pFileName, const u8 *pBuffer, i64 lFileSize, COORD crLoadSize)
	{
			...
			if (n && n < (MAX_PATH-16)) {
				...
				if (n) {
					...
					if (hFile == INVALID_HANDLE_VALUE) {
						szTempFile[0] = 0; // не создали, значит и удал€ть будет нечего
					} else {
		...
		if (!img) {
			//nErrNumber = gdi->nErrNumber; -- ошибка ”∆≈ в nErrNumber
	...
	bool GetPageBits(CET_LoadInfo *pDecodeInfo)
	{
		...
		else
		{
			...
			if (lbAllowThumb) {
				...
				// —разу пытаемс€ извлечь в режиме превьюшки (полна€ картинка нам не нужна)
